...
const int maxVertices = 1000;  // Maximum number of vertices (adjust as needed)
...
void dijkstra(Edge graph[][maxVertices], int source, int V) {
    ...
    for (int count = 0; count < V - 1; ++count) {
        ...
        for (int v = 0; v < V; ++v) {
            if (!processed[v] && graph[u][v].weight && distances[u] != INF && distances[u] + graph[u][v].weight < distances[v]) {
...
int main() {
    ...
    Edge graph[maxVertices][maxVertices];

    cout << "Enter the edges (source destination weight):\n";
    for (int i = 0; i < E; ++i) {
        int u, v, w;
        cin >> u >> v >> w;
        graph[u][v].destination = v;
        graph[u][v].weight = w;
        graph[v][u].destination = u;
        graph[v][u].weight = w;  // For an undirected graph
    }
